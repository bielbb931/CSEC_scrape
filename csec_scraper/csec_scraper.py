...
import os, re, io, typing
import requests
import pandas as pd
from dataclasses import dataclass
from playwright.sync_api import sync_playwright

# pdfminer import (robust across environments)
try:
    from pdfminer.high_level import extract_text
except Exception:  # very rare, but keeps things stable
    from pdfminer.high_level import extract_text
...
# Final Excel columns (Listing name first)
CANON_HEADERS = [
    "Listing name",
    "Certification ID",
    "Validity",
...
# Header aliases (English + Swedish; include exact FMV spellings)
ALIASES = {
    # Certification ID
    "certification id": "Certification ID",
    "certifierings id": "Certification ID",     # FMV spelling
    "certifikat id": "Certification ID",
    "certifikat-id": "Certification ID",
    "certifikatnummer": "Certification ID",
    "certifieringsid": "Certification ID",
    ...
    "försäkringspaket": "Insurance package",

    # Certification date
    "certification date": "Certification date",
    "certifieringsdatum": "Certification date",
    ...
    # Certification report (PDF link)
    "certification report": "Certification report",
    "certifieringsrapport": "Certification report",
}
...
class ProductRow:
    ...
    def to_dict(self):
        return {
            "Listing name": self.Listing_name,
            "Certification ID": self.Certification_ID or "NA",
            "Validity": self.Validity,
            "Product name": self.Product_name,
            "Product category": self.Product_category,
            "Insurance package": self.Insurance_package,
            "Certification date": self.Certification_date,
            "Developer": self.Developer,
            "ITSEF": self.ITSEF,
            "Product URL": self.Product_URL,
            "Certification report URL": self.Certification_report_URL,
        }

# ---------- LIST PAGE ----------
def _collect_list_links_with_titles(page) -> list[tuple[str,str]]:
    """
    Return [(href, listing_name)] from the two-column list page.
    Keep internal FMV/CSEC detail pages.
    """
...
def _extract_table_like_pairs(page) -> dict[str, dict]:
    """
    Build dict: canonical_header -> {'value': str, 'node': value_locator}
    Supports tables, definition lists, and "Label: Value" rows.
    """
...
def _value_after_label_block(page, label_variants: list[str]) -> str:
    """
    Robust fallback: find a node whose text equals any label (case-insensitive),
    then read the text content of the **first following element**. This matches
    FMV’s “label block then value block” layout (no table).
    """
...
# ---------- PDF: ITSEF extraction (PDF ONLY) ----------
import re, io, requests
from pdfminer.high_level import extract_text, extract_pages
from pdfminer.layout import LTTextBoxHorizontal, LTTextLineHorizontal

# Labels we expect in "2 Identification" that can delimit blocks
_IDENT_LABELS = [
...
# Regex: start-of-line label (case-insensitive), optional colon
_LABEL_START = re.compile(r"^\s*(%s)\s*:?\s*$" % "|".join(re.escape(x) for x in _IDENT_LABELS),
                          flags=re.IGNORECASE)

# “Noise” to exclude from values (your list + CC tokens)
_CC_TOKENS = r"\b(EAL|ALC|ADV|ATE|AVA|ASE|ACO|AGD)\b"
_EXCLUDE_NEARBY_PATTERNS = [
    r"\bdocument version\b",
    r"\bDeveloper\b",
    r"\bSponsor\b",
    r"\b3\.1\s*release\s*5\b",
    r"\bCommon\s+Criteria\s+version\b",
    r"\bCEM\s+version\b",
    r"\bQMS\s+version\b",
    r"\bSecurity\s+Target\b",
    r"\bTarget\b",
]
...
def _is_bad_value(v: str) -> bool:
    if not v:
        return True
    if re.search(_CC_TOKENS, v, re.IGNORECASE):
        return True
...
def _extract_itsef_text_mode(pdf_bytes: bytes) -> str:
    """Stage A: line-anchored text parser (no coordinates)."""
    txt = extract_text(io.BytesIO(pdf_bytes)) or ""
    txt = txt.replace("\u00ad", "")  # soft hyphen artifacts
    sec = _slice_identification_section(txt)
    lines = [ " ".join(l.split()) for l in sec.splitlines() ]

    # 1) Prefer exact start-of-line 'ITSEF' with value on the same line.
    for ln in lines:
        m = re.match(r"^\s*ITSEF\b[:\-\s]*(.+)$", ln, flags=re.IGNORECASE)
        if m:
            val = m.group(1).strip(" .;-")
            if not _is_bad_value(val):
                return val

    # 2) Standalone 'ITSEF' line -> collect following lines until next label.
    for i, ln in enumerate(lines):
        if re.fullmatch(r"\s*ITSEF\s*[:\-]?\s*", ln, flags=re.IGNORECASE):
            ...
            for j in range(i+1, min(i+8, len(lines))):
                ...
                if not nxt:
                    break
                # stop if the next line is clearly a label line
                if _LABEL_START.match(nxt):
                    break
                buf.append(nxt)
            val = " ".join(buf).strip(" .;-")
            # Sometimes first “next” line can be junk; trim obviously-bad heads
            if _is_bad_value(val) and len(buf) > 1:
                for k in range(len(buf)):
                    vv = " ".join(buf[k:]).strip(" .;-")
                    if not _is_bad_value(vv):
                        val = vv; break
            if val and not _is_bad_value(val):
                return val

    return ""  # let layout mode try

def _extract_itsef_layout_mode(pdf_bytes: bytes) -> str:
    """Stage B: layout-aware parsing – find the text to the right of the line that contains 'ITSEF'."""
    try:
                ...
                if isinstance(obj, LTTextBoxHorizontal):
                    for line in obj:
                        if isinstance(line, LTTextLineHorizontal):
                            ...
                            if not text:
                                continue
                            # x0, x1, y0, y1 tell us where the line sits
                            lines.append((text, line.x0, line.x1, line.y0, line.y1))
            # Look for the line containing 'ITSEF'
            for text, x0, x1, y0, y1 in lines:
                if re.search(r"\bITSEF\b", text, flags=re.IGNORECASE):
                    # collect text lines on the same row, to the right
                    row_vals = []
                    for t2, a0, a1, b0, b1 in lines:
                        same_row = abs(((y0 + y1) / 2) - ((b0 + b1) / 2)) < 2.0  # same baseline (tune if needed)
                        to_right = a0 > x1 + 1.0
                    ...
                    if row_vals:
                        ...
                        if candidate and not _is_bad_value(candidate):
                            return candidate
        return ""
    except Exception:
        return ""
...
def _extract_itsef_from_pdf(pdf_url: str) -> str:
    """Download PDF and run text-mode; if empty/wrong, run layout-mode."""
    if not pdf_url:
        return ""
    try:
    ...
    # Stage A: line-anchored (fast, robust when ITSEF is on same line)
    val = _extract_itsef_text_mode(pdf_bytes)
    if val and not _is_bad_value(val):
        return val

    # Stage B: layout-aware (use coordinates to read the right-hand cell)
...
def run(headless: bool = True, out_xlsx: str = "output/csec_products.xlsx") -> int:
    ...
    with sync_playwright() as p:
        ...
        for link, listing_name in link_title_pairs:
            ...
            # Certification ID: structured first...
            cert_id = pairs.get("Certification ID", {}).get("value", "")
            # ...then robust fallback (FMV “label then next block value” layout)
            if not cert_id:
